...
//
//   1. KEYED TO THE BRIEFING SLOT, NOT THE SECOND. SignalDesk publishes at
//      07:00 and 19:00 IST. A trade is described by the briefing that was in
//      effect when it was entered. Nothing analytically useful is lost — Fear
//      & Greed moves once a day, funding regimes persist — and a day+slot key
//      is reproducible, so a failed capture can be retried later.
//
//   2. AT OR BEFORE, NEVER AFTER. A trade entered at 06:00 IST maps to the
//      PREVIOUS evening's 19:00 briefing, not to the 07:00 one published an
...
// The two briefing slots, in IST hours. Order matters: latest first.
export const SLOT_HOURS = [19, 7];

// How far back to walk when the resolved slot has no stored digest (a cron
// failure). 4 slots = 2 days; beyond that the briefing is not "context for
// this trade" in any honest sense, so we return none rather than something.
const MAX_DIGEST_FALLBACK_SLOTS = 4;
...
// the latest 07:00/19:00 IST publication AT OR BEFORE it.
//   09:14 IST → today 07 · 19:00 IST → today 19 (a slot owns its own instant)
//   06:00 IST → YESTERDAY 19, never today's 07
...
export function resolveSlot(at = new Date()) {
  ...
  if (slotHour === undefined) {
    // Before 07:00 IST: the briefing in effect is the previous evening's.
    slotHour = 19;
    dayMs -= DAY_MS;
  }
...
// The slot immediately before this one. Used to walk back when a slot has no
// stored digest — backwards only, same no-lookahead rule.
...
// the API). Returns null for anything that isn't a real slot id, so a typo
// can't be mistaken for a valid day.
export function slotFromParts(date, slot) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(date || ''))) return null;
  const hour = Number(slot);
  if (!SLOT_HOURS.includes(hour)) return null;
  const [y, m, d] = date.split('-').map(Number);
  const dayMs = Date.UTC(y, m - 1, d);
  if (!Number.isFinite(dayMs)) return null;
  // Resolve from the slot's own instant so the result is normalized through
  // exactly the same code path as a trade timestamp.
  return resolveSlot(new Date(dayMs + hour * HOUR_MS - IST_OFFSET_MS));
}
