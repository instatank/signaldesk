...
Each headline has a "recent" flag: true means it landed within the briefing's recency window (recentWindowHours in the input), false means it is older background. Treat "recent" headlines as the primary story — lead with them in top_stories. Only reach for a non-recent headline if nothing recent is significant enough to fill the section.
...
- Write market_pulse as a right-now snapshot in the present tense: what has changed in the last few hours, and how price, funding, and sentiment are responding to it.
- Older (non-recent) items are background only — use them to explain the reaction, never to lead the briefing.
- If the last few hours are genuinely quiet, SAY SO plainly ("no major new catalysts in the last few hours") rather than manufacturing urgency. All standing rules still apply: no predictions, no trade calls, only the data provided, and flag thin or mixed data honestly.`;
}
