...
// Compact form of interpretOiPrice for the briefing's positioning grid —
// same four-quadrant logic, two words instead of a sentence. The grid is
// meant to be scanned, not read; the dashboard carries the full prose.
export function oiPriceTag(priceChangePct, oiChangePct) {
  if (priceChangePct == null || oiChangePct == null) return null;
  if (Math.abs(priceChangePct) < 0.5 || Math.abs(oiChangePct) < 1) return 'no clear flow';
  const priceUp = priceChangePct > 0;
  const oiUp = oiChangePct > 0;
  if (priceUp && oiUp) return 'new longs';
  if (priceUp && !oiUp) return 'short cover';
  if (!priceUp && oiUp) return 'new shorts';
  return 'longs out';
}
...
// --- Screener reads. All mechanical thresholds over price/funding/OI —
// same no-predictions contract. Labels mirror the reference screener.

// Trend from a coin's own price shape: position vs its 200-day average
// plus recent (7d) direction. Deliberately coarse — four buckets.
export function interpretScreenerTrend({ aboveMa200, r7d }) {
  if (aboveMa200 == null) return { label: 'unrated', tone: 'gray' };
  if (aboveMa200 && r7d >= 0) return { label: 'uptrend', tone: 'green' };
  if (aboveMa200 && r7d < 0) return { label: 'cooling', tone: 'amber' };
  if (!aboveMa200 && r7d > 3) return { label: 'bounce', tone: 'lime' };
  return { label: 'downtrend', tone: 'red' };
}

// A coin's 30-day return next to Bitcoin's. Context, not a signal.
export function interpretVsBitcoin(coinR30, btcR30, isBtc = false) {
  if (isBtc) return null;
  if (coinR30 == null || btcR30 == null) return null;
  const diff = coinR30 - btcR30;
  if (diff > 5) return 'ahead of BTC';
  if (diff < -5) return 'behind BTC';
  return 'tracks BTC';
}

// Cost to hold a long: the past-week average funding, annualized. Positive
// = longs pay (expensive); sufficiently negative = shorts pay you to hold.
export function interpretCostToHold(annualPct) {
  if (annualPct == null || !Number.isFinite(annualPct)) return { label: '—', tone: 'gray' };
  if (annualPct >= 10) return { label: `~${Math.round(annualPct)}%/yr`, tone: 'amber' };
  if (annualPct <= -5) return { label: 'paid to hold', tone: 'green' };
