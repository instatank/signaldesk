...
import { assembleDigestInputs, attachStoryLinks, buildProvenance } from './digest.js';
...
export async function leanIngest(db, sources, startedAt = new Date()) {
  const errors = [];
  ...
  if (rssResult.status === 'fulfilled') {
    ...
    await Promise.all(
      rssResult.value.items.map(async (item) => {
        try {
...
export async function runFlash(db, sources, macroEvents = [], startedAt = new Date()) {
  const ingest = await leanIngest(db, sources, startedAt);
  ...
  try {
    ...
    digest.meta = buildProvenance(inputs, startedAt);
  } catch (err) {
