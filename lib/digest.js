...
// call (and the raw fallback) consumes.
export async function assembleDigestInputs(db, assets) {
  const now = new Date();
  ...
  const headlines = headlinesSnap.docs.map((doc) => {
    const d = doc.data();
    return { title: d.title, source: d.source, publishedAt: d.publishedAt?.toDate?.()?.toISOString?.() || null };
  });
