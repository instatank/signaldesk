...
import { titleKey } from './dashboard.js';
...
export function istSlotId(date = new Date()) {
  // One doc per run, not per day. The schedule is once daily (07:00 IST) so
  // in practice this is "<date>-07", but the run-keyed id is kept: a manual
  // re-run must not overwrite the morning's stored briefing, and the archive
  // still holds the twice-daily "-19" docs from before 2026-08-29.
...
// appears in every briefing stops being read. Twice a week, on the daily
// 07:00 IST run (the hour check also keeps it off any manual re-run later
// in the day), and never on a flash (a flash is a market reaction, not a
...
//   recentHours  — the "lead with these" cutoff for headlines (default 24h,
//                  i.e. the whole window: with ONE briefing a day, every
//                  story in it is new to the reader. It was 12h back when
//                  the digest ran twice daily and the evening run had to
//                  avoid re-telling the morning's news.)
//   mode         — 'scheduled' (the daily digest) or 'flash' (on-demand,
//                  live-reaction run). Passed through to the prompt so the
//                  briefing is framed for its horizon.
// The defaults reproduce the scheduled digest exactly — the daily cron
// calls this with no opts.
export async function assembleDigestInputs(db, assets, macroEvents = [], opts = {}) {
  const { windowHours = 24, recentHours = 24, mode = 'scheduled' } = opts;
  const now = new Date();
  ...
  const headlinesSnap = await db
    ...
    .limit(60)
    .get();
  // Deduped by normalized title, not just by URL. Relayed sources (Tree News)
  // carry the newsrooms' stories minutes earlier under their own links, so
  // without this the same story reaches the model twice and reads as two
  // independent confirmations — precisely the mistake the conviction bar
  // exists to prevent. Done here rather than downstream so headline_index
  // (which attachStoryLinks resolves) indexes the list the model actually saw.
  const seenTitles = new Set();
  const headlines = [];
  for (const doc of headlinesSnap.docs) {
    const d = doc.data();
